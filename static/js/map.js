...
if (typeof (map_element) != 'undefined' && map_element != null) {
  ...
  for (const k in data) {
    ...
    const long = Number(caseStudy.long);
    const lat = Number(caseStudy.lat);
    if (isNaN(long) || isNaN(lat)) {
      console.warn(`Invalid coordinates for case study ${caseStudy.id}: ${caseStudy.lat}, ${caseStudy.long}`);
      continue;
    }
